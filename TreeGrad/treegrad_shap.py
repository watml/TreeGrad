...
def treegrad_shap(model, x, class_index=None):
    ...
    if hasattr(model, 'estimators_'):
        ...
        for i, stage in enumerate(model.estimators_):
            if shape[1] == 1:
                result[i] = treegrad_shap_(stage[0].tree_, x, 0)
            else:
                assert class_index is not None
                result[i] = treegrad_shap_(stage[class_index].tree_, x, 0)
        ...
        outcome = treegrad_shap_(model.tree_, x, class_index or 0)
        
    return outcome



def treegrad_shap_(tree, x, value_index):
